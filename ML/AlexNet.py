...
# %%
with open('AlexNet_log.txt', 'w', newline='') as recordFile:
    for lr_i in lr_list:
        for momentum_i in momentum_list:
            ...
            with open(f"Scores/AlexNet_lr{lr_i}_momentum{momentum_i}.csv", 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(["Train F1", "Test F1","Saved"])

            with open(f'Scores/AlexNet_lr{lr_i}_momentum{momentum_i}.csv', 'a', newline='') as f:
                writer = csv.writer(f)
                ...
                for epoch in tqdm(range(epochs)):  # Maximum number of epochs
                    ...
                    if test_f1 > prev_loss_f1:
                        torch.save(model.state_dict(), f'Model/AlexNet_lr{lr_i}_momentum{momentum_i}.pth')
